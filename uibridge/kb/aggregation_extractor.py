...
from .profile_extractor import FrameworkProfile

logger = logging.getLogger(__name__)


class AggregationExtractor:
    """Phase 2 aggregated extraction that only scans UI-relevant files."""

    def __init__(self, project_root: str, java_extractor, convention_miner):
        self.project_root = Path(project_root)
        self.java_extractor = java_extractor
        self.convention_miner = convention_miner
    ...
    def _extract_single_ui_file(self, filepath: Path,
        ...
        return self.java_extractor._legacy_extract_from_java_file(str(filepath))
    ...
    def _extract_aggregated_conventions(self, profile: "FrameworkProfile",
        ...
        op_item = self.convention_miner._extract_operation_conventions(referenced_files)
        if op_item:
            convention_items.append(op_item)

        # 2. Import patterns: commonly used import statements
        import_item = self.convention_miner._extract_import_conventions(referenced_files)
        if import_item:
            convention_items.append(import_item)

        # 3. Locator usage patterns: actual locator types used in source
        locator_item = self.convention_miner._extract_locator_usage_conventions(referenced_files, profile)
        if locator_item:
        ...
        if test_dir:
            ...
            if test_target.exists():
                ...
                assertion_item = self.convention_miner._extract_assertion_conventions(test_files)
                if assertion_item:
    ...
    def _extract_patterns(self, test_files: list[Path],
                          profile: "FrameworkProfile") -> list["KBItem"]:
        """从测试文件中挖掘 BAW 模式。"""
        # Delegates to convention miner for the actual pattern extraction
        return self.convention_miner._extract_patterns(test_files, profile)
