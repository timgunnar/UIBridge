"""Python extraction mixin — legacy Python extractors and AST helpers."""
...
class _PythonMixin:
    """Legacy Python extractors and Python AST helpers for KBExtractor."""
    ...
    def _extract_feature_points(self, tree: ast.Module) -> list[dict]:
        ...
        from ._base import _FEATURE_POINT_RE
        points = []
        for node in ast.walk(tree):
            if isinstance(node, ast.Assign):
                for target in node.targets if isinstance(node.targets, list) else [node.targets]:
